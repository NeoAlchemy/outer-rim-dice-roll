...
(function (dice) {
  ...
  this.create_dice_materials = function (face_labels, size, margin) {
    function create_text_texture(text, color, back_color) {
      ...
      let grad = back_color[0];
      if (back_color.length > 1) {
        grad = context.createLinearGradient(0, 0, 280, 0);
        for (let i = 0; i < back_color.length; i++) {
          grad.addColorStop(i / back_color.length, back_color[i]);
        }
      }
  ...
  this.dice_color = ['#D29951', '#B17F4B', '#A5714E'];
  //this.dice_color = ['#0F68C0', '#167C28', '#05204E'];
